...
- **가로 여백 활용 레이아웃(동적 반응형).** 콘텐츠는 좁은 단일 컬럼으로 세로만 길게 쌓지 말고 가로 폭을 쓴다. 컨테이너 폭은 개념·구현 트랙 모두 `max-width:1200px`으로 통일하고, **본문 텍스트는 폭 제한 없이 오른쪽 끝까지 꽉 채운다(글줄 캡 금지)**. 데모·표·코드·카드그리드·히어로도 전폭. **고정 열 수·미디어쿼리 대신 폭 기반(intrinsic) 반응형**을 쓴다: 카드/블록은 `grid-template-columns: repeat(auto-fit, minmax(<최소폭>, 1fr))`, 설명 리스트는 `columns: <최소폭>`(예: 320px)로 폭에 맞춰 열 수 자동 조절. **데모 섹션은 `[제목]→[텍스트 전폭]→[데모 아래 전폭]` 세로 배치로 통일(텍스트·데모를 좌우 2단 `.split`으로 나누지 않음).** 유틸: `.grid2`/`.grid3`/`.feature-grid`. 상세는 `docs/CONTENT-GUIDE.md`의 "레이아웃 규칙" 참고.
