...
- **가로 여백 활용 레이아웃.** 콘텐츠는 좁은 단일 컬럼으로 세로만 길게 쌓지 말고 넓은 화면의 가로 폭을 쓴다. 컨테이너 폭은 개념·구현 트랙 모두 `max-width:1200px`으로 통일하고, 본문 글줄은 읽기 좋은 폭(약 74~82ch)으로 제한하되 데모·표·코드·카드그리드·히어로는 전폭을 쓴다. 가로 분할이 필요하면 `.grid2`/`.grid3`/`.split` 유틸을 쓴다. 상세는 `docs/CONTENT-GUIDE.md`의 "레이아웃 규칙" 참고.
