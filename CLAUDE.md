...
- **impl 페이지 상단 내비 표준(통일).** 모든 `impl/*.html`은 **①좌상단 뒤로가기**를 `<main>` 첫 줄에 둔다:
  `<a class="back-link" href="./index.html" onclick="if(history.length>1){history.back();return false}">← 뒤로가기</a>`.
  **②브레드크럼(`.impl-topbar`)은 항상 동일한 4단**으로 통일한다: `개념·데모 가이드 | 구현 허브 | N장.제목(<span> var(--muted)) | 현재(var(--accent2))`. 중간 서브허브 레벨(예: "렌더링 경로")을 끼우지 말고, 사례연구 하위 페이지도 이 형식으로 맞춘다. 상세는 `docs/CONTENT-GUIDE.md` §2(0·0-b).
