...
- **가로 여백 활용 레이아웃(동적 반응형).** 콘텐츠는 좁은 단일 컬럼으로 세로만 길게 쌓지 말고 가로 폭을 쓴다. **고정 열 수·미디어쿼리 브레이크포인트 대신, 가로 폭을 스스로 인식하는 폭 기반(intrinsic) 반응형을 쓴다**: 카드/블록은 `grid-template-columns: repeat(auto-fit, minmax(<최소폭>, 1fr))`, 설명 리스트는 `columns: <최소폭>`(예: 320px). 그러면 컨테이너 폭에 맞춰 열 수가 자동으로 늘고 준다. 컨테이너 폭은 개념·구현 트랙 모두 `max-width:1200px`으로 통일하고, 본문 문단만 읽기 좋은 폭(약 74~82ch)으로 제한하되 데모·표·코드·카드그리드·히어로는 전폭을 쓴다. 유틸: `.grid2`/`.grid3`/`.feature-grid`(auto-fit 카드), `.split`. 상세는 `docs/CONTENT-GUIDE.md`의 "레이아웃 규칙" 참고.
